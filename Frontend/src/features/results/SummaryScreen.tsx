...
export function SummaryScreen() {
  ...
  return (
    <main className="mx-auto flex max-w-lg flex-col items-center gap-4 p-6 pt-24 text-center">
      <h1 className="text-4xl font-semibold tracking-wide text-ivory">Final Score</h1>
      <div className="marble-panel flex w-full flex-col items-center gap-6 rounded-sm border border-gold/40 p-8">
        ...
        <div className="flex w-full flex-col gap-2">
          {summary.rounds.map((round, index) => (
            <div
              key={round.roundId}
              className="flex items-center justify-between rounded-sm border border-gold/25 bg-ink-soft px-4 py-2 text-sm text-ivory"
            >
